...
function loadLocalStorageStep2() {
    if (localStorage.getItem("listSpace2") !== null) {
        ...
        if (data.exceptions) {
            $(`input[name="exceptions"][value="${data.exception}"]`).prop('checked', true);
        }
    }
}
...
function moveToNextStepFromStep2() {
    ...
    if (!spaceName || !typeOfSpace || !description || !openingDate|| !availableDaysFrom || !availableDaysTo || !operatingHoursFrom || !operatingHoursTo || !email || !phone || !instagram || !facebook || !contactNo ) {
        Swal.fire({
            icon: "error",
